              ...
              <CardContent>
                <div className="space-y-4">
                  {topicAnalysisData.slice(0, 5).map((topic, index) => (
                    <div key={index} className="flex items-center justify-between bg-slate-900/40 p-3 rounded-lg border border-slate-700/50">
                      ...
                      <div className="flex items-center space-x-3">
                        <Badge
                          className={`${
                            topic.sentiment > 0.5 ? 'bg-green-500/20 text-green-400 border border-green-500/40' :
                            topic.sentiment < -0.2 ? 'bg-red-500/20 text-red-400 border border-red-500/40' :
                            'bg-yellow-500/20 text-yellow-400 border border-yellow-500/40'
                          }`}
                        >
                          {(topic.sentiment * 100).toFixed(0)}%
                        </Badge>
                        <div className={`text-xs font-medium flex items-center ${topic.growth > 0 ? 'text-green-400' : 'text-red-400'}`}>
                          {topic.growth > 0 ? <TrendingUp className="w-3 h-3 mr-1" /> : <TrendingDown className="w-3 h-3 mr-1" />}
          ...
          >
            <Card className="bg-slate-800/90 backdrop-blur-md border border-slate-700 shadow-xl hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300">
              <CardHeader>
                ...
                <CardDescription className="text-slate-400">Analysis of sentiment/engagement synergy</CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <div className="text-xs text-slate-500 mt-2 text-center bg-slate-900/50 p-2 rounded-lg border border-slate-700/50">
                  Core Correlation Factor: <span className="text-orange-400 font-medium">0.78</span> (High Synergy)
                </div>
              </CardContent>
            </Card>
          </motion.div>
          ...
          >
            <Card className="bg-slate-800/90 backdrop-blur-md border border-slate-700 shadow-xl hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300">
              <CardHeader>
                ...
                <CardDescription className="text-slate-400">Autonomous intelligence summaries</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                    ...
                    <div className="text-xs text-slate-400 mt-1 leading-relaxed">
                      AI adoption discussions surged by 45% in active data streams.
                    </div>
                  </div>
                  <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-xl">
                    ...
                    <div className="text-xs text-slate-400 mt-1 leading-relaxed">
                      Ecosystem sentiment improved by 12% across primary nodes.
                    </div>
                  </div>
                  <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                    ...
                    <div className="text-xs text-slate-400 mt-1 leading-relaxed">
                      Data integrity concerns showing varied sentiment responses.
                    </div>
                  </div>
                </div>
              </CardContent>
